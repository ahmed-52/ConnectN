...
class Board:
    ...
    def __init__(self,rows=6,cols=7,streak=4):
        ...
        assert isinstance(streak,int) and\
         streak > 0 and streak <= min(rows,cols)
        ...
        for i in range(self._height):
            row = [NOBODY for n in range(self._width)]
            board.append(row)
        self._board = board
        self._moves = []
        self.clear()
    ...
    def getColor(self,r,c):
        ...
        """
        assert isinstance(r,int) and (0 <= r < self._height)
        assert isinstance(c,int) and (0 <= c < self._width)
    ...
    def findAvailableRow(self,col):
        ...
        """
        assert isinstance(col,int) and (0 <= col < self._width)
        for i in range(self._height):
    ...
    def place(self,column,color):
        ...
        """
        assert isinstance(column,int) and (0 <= column < self._width)
        assert isinstance(color,str) and \
        introcs.is_webcolor(color) or introcs.is_tkcolor(color)

        av = self.findAvailableRow(column)
        if av == self._height:
            return -1
        self._board[av][column] = color
        self._moves.append((av,column))
        return av
...
#### TASK 4 ####
    ...
    def findAcross(self,r,c,leng):
        ...
        c2 = c
        #left side of board
        while (c2 <= self._width -1) and (self._board[r][c2] == color):
            c2 = c2 + 1
        c2 = c2 - 1
        c3 = c
    ...
    def findSWNE(self,r,c,leng):
        ...
        while ((c2 <= self._width-1) and 
        (r2 <=self._height -1) and (self._board[r2][c2] == color)):
            c2 = c2 + 1
            r2 = r2 + 1
        c2 -= 1
        r2 -= 1
        ...
        while (c3 >= 0) and (r3 >= 0) and (self._board[r3][c3] == color):
            c3 = c3 - 1
            r3 = r3 - 1
        c3 += 1
        r3 += 1
    ...
    def findNWSE(self,r,c,leng):
        ...
        while( (r2 >=0) and (c2<=self._width-1) and 
        (self._board[r2][c2] == color)):
            r2 -= 1
            c2 += 1
        r2 += 1
        c2 -= 1
        ...
        r3 = r
        c3 = c
        while ((r3 <=self._height -1) and (c3 >= 0) and 
        (self._board[r3][c3] == color)):
            r3 += 1
            c3 -= 1
        r3 -= 1
        c3 += 1
        if dist(self,r2,c2,r3,c3) >= leng:
            return (r3,c3,r2,c2)
        else:
            return None
    ...
    def findWins(self,r,c):
        ...
        assert in_range(self,r,c)
        assert self._board[r][c] != NOBODY
        #lastR ,lastC = self.getLastMove()
       # assert lastR == r and lastC == c
        ver = self.findVertical(r,c,self.getStreak())
        acr = self.findAcross(r,c,self.getStreak())
        swe = self.findSWNE(r,c,self.getStreak())
        nwe = self.findNWSE(r,c,self.getStreak())
